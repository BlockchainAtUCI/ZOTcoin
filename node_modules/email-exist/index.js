...
function setLoggerStatus(options)
{
    if (options.loggerStatus) {
...
module.exports.verify = function verify(email, options, callback) {
    ...
    setLoggerStatus(params.options);
...
function beginSMTPQueries(params) {
    ...
    logger.info("Creating connection... to port " + params.options.port);
    let socket = net.createConnection(params.options.port, params.options.smtp);

    let callback = (err, object) => {
        callback = () => {
        }; // multiple sources could call the callback, replace the function immediately to prevent it from being called twice
        ended = true;
            ...
            switch (stage) {
                case 0:
                    if (response.indexOf('220') > -1 && !ended) {
                        // Connection Worked
                        let cmd = 'EHLO ' + params.options.fqdn + '\r\n';
                        logger.client(cmd);
                ...
                case 1:
                    if (response.indexOf('250') > -1 && !ended) {
                        // Connection Worked
                        let cmd = 'MAIL FROM:<' + params.options.sender + '>\r\n';
                        logger.client(cmd)
                        socket.write(cmd, function () {
                            stage++;
                            response = '';
...
function changePort(params)
{
    if (params.options.port !== 2525) {
        let currentPortIndex = smtpAlternativePorts.indexOf(params.options.port);
        params.options.port = smtpAlternativePorts[currentPortIndex+1];
        logger.info('Try connect to port ' + params.options.port);
        beginSMTPQueries(params);
    }
}